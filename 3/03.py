# 카드  역배치(정올  기출)
# 1부터  20까지  숫자가  하나씩  쓰인  20장의  카드가  아래  그림과  같이  
# 오름차순으로  한  줄로  놓여있다.  
# 각  카드의  위치는  카드  위에  적힌  숫자와  같이  1부터  20까지로  나타낸다. 
# 이제  여러분은  다음과  같은  규칙으로  카드의  위치를  바꾼다:  구간  [a,  b]  
# (단,  1  ≤  a  ≤  b  ≤ 20)가  주어지면  위치  a부터  위치  b까지의  카드를  
# 현재의  역순으로  놓는다.
# 예를  들어,  현재  카드가  놓인  순서가  위의  그림과  같고  
# 구간이  [5,  10]으로  주어진다면,  
...
def calculate(reverse_list):
  card_one_twenty=list(range(1, 21))
  print(card_one_twenty)
  for i in reverse_list:
    temp_list = card_one_twenty[int(i[0])-1: int(i[1])]
    reversed_temp_list= temp_list[::-1]
    for k in range(int(i[0])-1 , int(i[1])):
      card_one_twenty[k]=reversed_temp_list[k-int(i[0])+1]
  print(card_one_twenty)
